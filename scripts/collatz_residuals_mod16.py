#!/usr/bin/env python3
"""
Analyze regression residuals grouped by starting number modulo 16.
...
- Group by n mod 16 (residue classes 0-15)
...
def main():
    ...
    # Group residuals by n mod 16
    residual_groups = {i: [] for i in range(16)}
    
    max_n = 10_000_000
    
    print(f"Analyzing residuals for n = 1 to {max_n:,} (mod 16)...")
    ...
    for residue_class in range(16):
        ...
        if count > 0:
            ...
            results[f"mod16_{residue_class}"] = {
    ...
    output_file = "collatz_residuals_mod16_10000000.json"
    ...
    # Print summary
    print("\nRESIDUAL ANALYSIS BY n mod 16:")
    print("=" * 70)
    for residue_class in range(16):
        stats = results[f"mod16_{residue_class}"]
        print(f"n ≡ {residue_class:2d} (mod 16): mean={stats['mean_residual']:8.4f}, "
