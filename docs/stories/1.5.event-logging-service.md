...
- [ ] **Task 1: Create LogRecord Dataclass and EventType Enum** (AC: 3)
  - [ ] Create file `contreact_ollama/logging/jsonl_logger.py`
  - [ ] Define `EventType` enum with all event types (CYCLE_START, LLM_INVOCATION, TOOL_CALL, CYCLE_END)
  - [ ] Define `LogRecord` dataclass with timestamp, run_id, cycle_number, event_type, payload
  - [ ] Add type hints and docstrings
  - [ ] Import datetime for ISO 8601 timestamps

- [ ] **Task 2: Implement JsonlLogger Class** (AC: 1, 3)
  - [ ] Implement `__init__(log_file_path: str)` method
  - [ ] Open log file in append mode
  - [ ] Implement `log_event()` method to write log records
  - [ ] Generate ISO 8601 timestamps
  - [ ] Serialize LogRecord to JSON and write with newline
  - [ ] Add proper file handling and closing

- [ ] **Task 3: Implement Log File Path Management** (AC: 1)
  - [ ] Create `logs/` directory if it doesn't exist
  - [ ] Generate log file path from run_id: `logs/{run_id}.jsonl`
  - [ ] Ensure directory creation uses Path.mkdir(parents=True, exist_ok=True)

- [ ] **Task 4: Integrate Logger with CycleOrchestrator** (AC: 2)
  - [ ] Update CycleOrchestrator.__init__() to accept logger parameter
  - [ ] Log CYCLE_START event at beginning of each cycle
  - [ ] Log CYCLE_END event at end of each cycle
  - [ ] Include cycle_number in all log events

- [ ] **Task 5: Update ExperimentRunner to Initialize Logger** (AC: 1, 2)
  - [ ] Update initialize_services() to create JsonlLogger
  - [ ] Pass run_id from config to determine log file path
  - [ ] Add logger to services dict
  - [ ] Pass logger to CycleOrchestrator constructor

- [ ] **Task 6: Testing** (AC: 1, 2, 3)
  - [ ] Write unit tests for JsonlLogger
  - [ ] Test log_event() creates valid JSON lines
  - [ ] Test file creation and append mode
  - [ ] Test ISO 8601 timestamp generation
  - [ ] Integration test logging during experiment run
  - [ ] Validate .jsonl file format manually
